...
function layoutHTML5QuizPlayer() {
    ...
    $j(".quizContent").append('<a role="navigation" class="pageArrowForward" id="forward-page" href="javascript:void(0)">Next</a>');
    $j(".quizContent").append('<a role="navigation" class="pageArrowBack" id="back-page" href="javascript:void(0)" aria-hidden="true">Back</a></div>');
...
function generateButtons() {
    ...
    for(; i < length; ++i) {
        ...
        var appendString = "<li><a class='button " + isAnswered + "' href='javascript:void(0)' aria-label=' "+ label + "'>" + ((addSpan != "") ? addSpan : (i + 1)) + "</a></li>";
        $j(".quizNav" ).append(appendString);
    }
    $j(".quizNav").append("<li><a id='done' class='button doneOff' aria-disabled='true' href='javascript:void(0)'>Done</a></li>");
...
function generateButtonsForReadOnly() {
    ...
    for(; i < length; ++i) {
        ...
        var appendString = "<li><a class='button " + isAnswered + "' href='javascript:void(0)'  aria-disabled='true'>" + (i + 1) + "</a></li>";
        $j(".quizNav" ).append(appendString);
    }
    $j(".quizNav").append("<li><a class='button doneOff' href='javascript:void(0)' >Done</a></li>");
}
    ...
    function redirectToActivityRewardPage() {
        if(typeof BadgeNotificationManager !== 'undefined') {
            ...
            manager.onNextBadgeDisplayComplete(function() {
                window.location.replace('/main/ActivityReward/id/' + $j('#quizResourceDeploymentId').val());
            })
        } else {
            window.location.replace('/main/ActivityReward/id/' + $j('#quizResourceDeploymentId').val());
        }
...
function determineAndSetResultsHtml(data) {
    ...
    if(!fromAssessment) {
        if(!data.is_passed) {
            resultsImg = "<img class='robot-noStars' src='/images/robot-fail.png' alt='Nice Try. Read the book again or try again.' />";
        }
        else {
            if(!data.is_perfect) {
                if(data.was_passed) {
                  resultsImg = "<img class='robot-noStars' src='/images/robot-passed-noStars.png' alt='Good Job' />";
                } else {
                  resultsImg = "<img src='/images/robot-passed.png' alt='Nice Job!' /><span class='numStars'>"+starsEarned+"</span>";
                  }
                } else {
                  if(data.was_perfect) {
                    resultsImg = "<img alt='Awesome!' class='robot-noStars' src='/images/robot-perfect-noStars.png' />";
                } else {
                  resultsImg = "<img alt='Awesome! You earned stars!'  src='/images/robot-perfect.png' /><span class='numStars'>"+starsEarned+"</span>";
                }
            }
        }
    } else {
      resultsImg = "<img alt='Awesome! You earned stars!' src='/images/robot-assessment.png' /><span class='numStars'>"+starsEarned+"</span>";
    }
...
function changeAnswers(questionIndex) {
    ...
    if(correct_answer_id){
        ...
        for(; i < numChoices; ++i) {
            ...
            var appendString = "<li><a  role='button' aria-disabled="+ currentQuestionCorrect +" id="+answerLinkId+" class='answerBtnLink' href='javascript:void(0)'";

            appendString += (isCurrentQuestionCorrect()) ? "><span  class='answersBtn disabled'>" : "><span class='answersBtn'>";
            appendString += String.fromCharCode(65 + i) +  "</span><span class='visuallyHidden'>"+ answerText + "</span></a> ";
...
function resetResultsDiv() {
    ...
    if (fromAssessment) {
        $j("#results").append("<div id='resultsScore' tabindex='0'><p id='resultsAudioString'></p></div>");
        setupBenchmarkRewardPage();
    } else {
        $j("#results").append("<div id='resultsScore' tabindex='0'><h1></h1><p id='resultsAudioString'></p></div>");
    }
    ...
    if (listenInfo.resourceDeploymentId && !fromAssessment) {
        $j("#results").append(
            "<a aria-label='listen' class='listen activityIconContainer activityIconContainer-lg' href='"+listenInfo.href+assignmentParam+"'>" +
        ...
        if (listenInfo.completionStatus == "true") {
            $j(".activityIconContainer.listen").addClass("activityIconContainer-perfect").append(
                "	<img alt='perfect' class='icon-perfect' src='/shared/images/icons/icon-perfect.svg' style='display: block;' />");}}

    if (readInfo.resourceDeploymentId && !fromAssessment) {
        $j("#results").append(
            "<a aria-label='read' class='read activityIconContainer activityIconContainer-lg' href='"+readInfo.href+assignmentParam+"'>" +
        ...
        if (readInfo.completionStatus == "true") {
            $j(".activityIconContainer.read").addClass("activityIconContainer-perfect").append(
                "	<img class='icon-perfect' src='/shared/images/icons/icon-perfect.svg' style='display: block;' />"
            );
        }
    }
    ...
    if (quizInfo.resourceDeploymentId && !fromAssessment) {
        if (quizResults.isPerfect == "" && (quizResults.isPassed == "1" || quizResults.isPassed == "")) {
            $j("#results").append(
                "<a aria-label='quiz' class='quiz activityIconContainer activityIconContainer-lg' href='#' onclick='goToFirstIncorrectQuestion(); return false;'>" +
                "	<span class='icon icon-quizC'></span>" +
                "</a>"
            );
        } else {
            $j("#results").append(
                "<a aria-label='quiz' class='quiz activityIconContainer activityIconContainer-lg' href='"+quizInfo.href+assignmentParam+"'>" +
                "	<span class='icon icon-quizC'></span>" +
                "</a>"
            );
        }
        ...
        if (quizInfo.completionStatus == "perfect" || quizResults.isPerfect) {
            $j(".activityIconContainer.quiz").addClass("activityIconContainer-perfect").append(
                "	<img alt='perfect quiz score' class='icon-perfect' src='/shared/images/icons/icon-perfect.svg' style='display: block;' />"
            );
            ...
            $j(".activityIconContainer.quiz").addClass("activityIconContainer-complete").append(
                "	<img alt='quiz completed' class='icon-complete' src='/shared/images/icons/icon-complete.svg' style='display: block;' />"
            );
        }
    }
    $j("a[class*='activityIconContainer']").wrapAll( "<div class='activity-actions' />");
}
