...
(function(ns) {
    var contentUrl = null;
    ns.setContentUrl = function(contentUrlIn){
        contentUrl = contentUrlIn
    };
    ns.getContentUrl = function(){
	...
	ns.embedHTML5Video = function(videoFileName, poster, width, height, autoPlay, allowFullScreen, replaceElem, trackingInfo, captionfiles, errorCallback, setVideoCompleteCallback, disallowStudentDownloadOnPlay) {
		var contentUrl = ns.getContentUrl();
	    var videoElem = new Element('video', { width: width, height: height, controls: 'controls', crossorigin: 'anonymous'} );

		if (poster != null) {
			videoElem.setAttribute('poster', contentUrl + poster);
		}
		...
		var sourceElem = new Element('source', { src: contentUrl + videoFileName, type: 'video/mp4' });
        ...
        if(captionfiles && captionfiles.length > 0){
            ...
            for (index = 0; index < captionfiles.length; index++) {
                var trackAttributes = {
                    src: contentUrl + '/videocontent/' + captionfiles[index].file,
                    kind: 'captions',
                    srclang: captionfiles[index].abbrev,
                    label: captionfiles[index].language_name
                };
                if(captionfiles[index].abbrev === "en"){
                    trackAttributes.default = "default";
                }
                videoElem.appendChild(new Element('track', trackAttributes));
            }
        }
        ...
        videoElem.addEventListener('error', function(event) {
            if(event.target.tagName.toUpperCase() === "TRACK"){
                var track = event.target;
                var trackErrorMessage = "Could not Load Text Track [" + track.src + "]";
                // IE has a bug in support of Cross-Domain Closed-Captions, so this error will be reached in IE 10 and IE 11
                isIE() ? console.log(trackErrorMessage) : console.error(trackErrorMessage);
                track.remove();
                return;
            }
        	errorCallback();
    ...
    ns.initializeSimplePopouts = function() {
        $$('a[rel="simplePopout"]').each(function(item) {
