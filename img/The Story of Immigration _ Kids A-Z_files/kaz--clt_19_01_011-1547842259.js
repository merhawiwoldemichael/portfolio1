...
function changeTeacherFromElt(elt) {
    ...
    if (newTeacherId.length > 0) {
        ...
        var uiRoutingHashPosition = newUrl.indexOf('#!');
        if (uiRoutingHashPosition >= 0) {
            var routing = newUrl.slice(uiRoutingHashPosition);
            newUrl = newUrl.slice(0,uiRoutingHashPosition);
            newUrl = window.clg.razKids.replaceUrlParam(newUrl, "teacher", newTeacherId);
            newUrl += routing;
        }
        else
            newUrl = window.clg.razKids.replaceUrlParam(newUrl, "teacher", newTeacherId);
...
function changeUserIcon(iconCell, iconID) {
    ...
    var thumbnailSource = "/images/login-icons-2018/"+iconID + ".png";
